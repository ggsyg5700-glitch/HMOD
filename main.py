...
PORT = 8081
_explicit_url = (os.getenv("WEBAPP_URL") or os.getenv("PUBLIC_URL") or "").strip().rstrip("/")
if _explicit_url:
...
def load_json(path, default):
    with _db_lock:
        with _db_conn() as conn, conn.cursor() as cur:
            ...
            if row is not None:
                return row[0]
            # seed from existing JSON file if available, else default
            seed = default
...
def require_admin(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if token != "admin_token_secure_123":
...
def api_auth():
    ...
    if data and data.get('password') == "admin123":
        return jsonify({"success": True, "token": "admin_token_secure_123"})
