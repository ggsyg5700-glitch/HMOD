...
window.confirmApproveOrder = function(id, username, price, itemName, transId) {
    ...
    window._pendingIsRecharge = isRecharge;
    ...
    document.getElementById('approve-modal-item').textContent = itemName || 'طلب';
    ...
    const amountSection = document.getElementById('approve-credit-section');
    ...
    if (isRecharge) {
        ...
        if (amountInput) { amountInput.value = price || ''; amountInput.style.borderColor = '#28a74566'; }
        if (modalTitle) modalTitle.textContent = 'تأكيد الموافقة على الشحن';
    } else {
        if (amountSection) amountSection.style.display = 'none';
        if (amountInput) amountInput.value = price || '';
        if (modalTitle) modalTitle.textContent = 'تأكيد الموافقة على الطلب';
    ...
    modal1.show();
    setTimeout(() => { if (isRecharge && amountInput) amountInput.focus(); }, 400);
};

window._approveStep2 = function() {
    const isRecharge = window._pendingIsRecharge;

    if (isRecharge) {
        const amountInput = document.getElementById('approve-credit-amount');
        const creditAmount = parseFloat(amountInput ? amountInput.value : 0);
        if (!creditAmount || creditAmount <= 0) {
            if (amountInput) { amountInput.style.borderColor = '#dc3545'; amountInput.focus(); }
        ...
        document.getElementById('approve-modal-confirm-text').textContent = 'أنت على وشك الموافقة على طلب شحن رصيد';
    } else {
        window._pendingCreditAmount = null;
        document.getElementById('approve-modal-username2').textContent = '@' + (window._pendingApproveUsername || 'N/A');
        document.getElementById('approve-modal-price2').textContent = '';
        document.getElementById('approve-modal-confirm-text').textContent = 'أنت على وشك الموافقة على الطلب';
    }
